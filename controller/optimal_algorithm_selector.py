...
class AlgorithmSelector:
    ...
    def optimal_route(self):
        ...
        a_star_elevation = utils.calculate_path_elevation_gain(self.graph, a_star_path)
        djikistra_path = self.djikistra.shortest_path()
        djikistra_elevation = utils.calculate_path_elevation_gain(self.graph, djikistra_path)
        print(a_star_elevation, djikistra_elevation)
        if self.elevation_gain=="max":
            if djikistra_elevation>a_star_elevation:
                return djikistra_path
            else:
                return a_star_path
        else:
            if djikistra_elevation>a_star_elevation:
